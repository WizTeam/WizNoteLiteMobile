...
class SwipeRow extends Component {
    ...
    deleteRow(callback) {
      ...
      this.setState({}, () => {
        Animated.timing(this.deleteRowAnimationValue, {
          toValue: 0,
          duration: 250,
          useNativeDriver: this.props.useNativeDriver,
        }).start(() => {
          this.deleteRowAnimationValue = null;
          console.log('delete row done');
          this.props.onRowDidDeleted && this.props.onRowDidDeleted();
            if (callback) {
              callback();
            }
        });
    ...
    render() {
      ...
      if (this.deleteRowAnimationValue) {
        heightStyle.height = this.deleteRowAnimationValue;
        heightStyle.overflowY = 'hidden';
      }

      // console.log('hiddenHeight', this.state.hiddenHeight);

        return (
            <Animated.View
                style={[this.props.style ? this.props.style : styles.container, heightStyle]}
